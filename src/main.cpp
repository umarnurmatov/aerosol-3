...
void setup() 
{
    ...
    if(IS_GPS_PRESENT) Serial1.begin(GPS_BAUD, SERIAL_8N1, GPS_RX_PIN, GPS_TX_PIN);
...
void loop() 
{
    ...
    if (readPM(pm10, pm25) && (!IS_GPS_PRESENT || isGPSDataValid())) 
    {
        ...
        if (millis() - timet >= FILE_WRITE_PERIOD) 
        {
            ...
            if(IS_BME_PRESENT) 
                strdata += String(bmeAlttitude) + "," + String(humidity) + "," + String(pressure) + "," + String(temperature);
            else 
...
static bool initFile()
{
    if(IS_GPS_PRESENT)
    {
        ...
        filename = "/" + String(date.month()) + "-" + String(date.day()) + "-" 
                    + String(date.year()) + "$" + String(time.hour())+ "-" + String(time.minute())
                    + "-" + String(time.second()) + ".csv";
    }
    else
    {
        int counter = 0;
        do { counter++; filename = "/" + String(counter) + ".csv"; }
